...
instance LoadableState AppState where
  load_state handle structure id = do
    let sql = build_select_multi ["Int__0", "ZMZN__1__id", "ZMZN__2__id"] "AppState" ("id = " ++ show id)
    putStrLn sql
    r <- execStatement handle $ sql
    let (total_length, words_id, latest_word_id) = case r of
    ...
    return $ AppState (read total_length) words latest_word
